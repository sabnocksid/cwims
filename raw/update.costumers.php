...
$id = $_GET['id'];
$sql = "SELECT * FROM costumers WHERE costumers_id=$id";
...
if (isset($_POST['submit'])) {
  ...
  if (preg_match('/^[a-zA-Z ]+$/', $name) && preg_match('/^[0-9]+$/', $contact) && preg_match('/^[a-zA-Z ]+$/', $address) ) {
        $sql = "UPDATE costumers SET name='$name', address='$address', contact='$contact' WHERE costumers_id=$id;";
      
        if ($conn->query($sql) === TRUE) {
          echo '<script>alert("Customer information updated successfully"); window.location.href="../costumers.php";</script>';
        } else {
