...
if(isset($_POST['submit'])){
    ...
    while ($row = mysqli_fetch_assoc($pid)) {
        ...
        $pid = mysqli_query($con,"SELECT product_id FROM product WHERE product_id='$productID' ");
        while ($row=mysqli_fetch_assoc($pid)) {
          mysqli_query($con,"INSERT INTO sales(product_id,quantity,amount,date_added,costumers_id) VALUES('{$row['product_id']}', '$quantity','$amount','$date','$cosID')  ") 
...
if ($result->num_rows > 0) {
    while ($row = mysqli_fetch_assoc($result)) {
        echo '<option value="' . $row['costumers_id'] . '">' . htmlspecialchars($row['name']) . '</option>';
    }
} else {
    ...
    <script>
        $(function() {
            $('#products').change(function() {
                ...
                var finalPrice = selectedPrice * 1.15;
